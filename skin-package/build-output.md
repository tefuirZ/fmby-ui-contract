...
`dist/index.html` 不是 immutable 资源。fmby 会在 HTML 里注入运行时 bootstrap，并且它引用的是当前构建生成的 hashed chunk。

要求：

1. 服务端必须对 SPA HTML 返回 `Cache-Control: no-store` 或等价禁止缓存策略。
2. skin 不应在 HTML 上配置长期缓存。
3. 升级后如果用户浏览器继续执行旧 chunk，优先检查 HTML 缓存和服务端主题注册是否重新加载。

hashed JS/CSS/图片可以长期缓存；入口 HTML 不能长期缓存。
