...
**漏带 CSRF token 的写操作**：返回 `403 Forbidden` + `error_code: csrf_missing`。
...
stage12U 期间 fmby 暂未引入限流。后续 active feature 后会在响应头加：
...
当前所有 API 响应**字段值是英文 / 数据原貌**——错误消息也是英文短语（如 `"library not found"`）。

skin 负责自己的 i18n（用 `error_code` 做 key，自己映射成本地化文案）。
