...
| 文档 | 范围 | 关键端点示例 |
|------|------|---------------|
| [dashboard.md](./dashboard.md) | 旧站点概览 / 高级状态 | `GET /api/manage/overview`、`GET /api/manage/advanced` |
| [operations.md](./operations.md) | 管理首页运营看板 | `GET /api/manage/operations/overview?days=30` |
...
- 所有响应遵循 [api/conventions.md](../../conventions.md)：成功直接返回 DTO，失败返回扁平 `ApiErrorResponse`。
...
- 皮肤可探测 bootstrap / `GET /api/site/bootstrap` 中的 `features.pan115_imghost_enabled` 展示图床服务可用性，但不应把图床治理页直接隐藏或改成 404；后端未启用时页面内展示配置引导。
